...
<c:set var="customerRecID" value="no identity object found" scope="session"/>
<c:if test="${identity != null && identity.user.displayName != null}">
	<c:set var="customerRecID" value="searched for ${identity.user.displayName}" scope="session"/>
	...
	<c:if test="${record != null && record.get('customerID') != null}">
		<c:set var="customerRecID" value="${record.get('customerID')}" scope="session"/>
	</c:if>
</c:if>
